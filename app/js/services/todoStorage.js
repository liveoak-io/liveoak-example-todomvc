...
todomvc.factory('todoStorage', function (LiveOak, $rootScope) {
    ...
    return {
        query: function (query, success, error) {
            LiveOak.app().readMembers('/storage/todos', { query: query, sort: 'user,title', success: wrapSuccess(success, 200), error: wrapError(error) });
        },

        remove: function (todo, success, error) {
            LiveOak.app().remove('/storage/todos', todo, { success: wrapSuccess(success, 200), error: wrapError(error) });
        },

        save: function (todo, success, error) {
            LiveOak.app().save('/storage/todos', todo, { success: wrapSuccess(success, 201), error: wrapError(error) });
        },

        update: function (todo, success, error) {
            LiveOak.app().update('/storage/todos', todo, { success: wrapSuccess(success, 200), error: wrapError(error) });
        }
    };
});